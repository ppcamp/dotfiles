...
alias cls='clear'
alias hist='history'
alias histclear='echo "" > ~/$HISTFILE && fc -R'
...
alias diff='git diff --no-index --word-diff=porcelain'
alias difff='git diff --no-index --word-diff=color'
...
### yq/jq (json procesor)
alias yjq='yq -Poj'
alias sortxml='yq -p xml -o xml "sort_keys(...)"'

#### ai
alias oclaude='openclaude' # see claude
