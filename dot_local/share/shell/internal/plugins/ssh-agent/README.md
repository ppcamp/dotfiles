...
```zsh
zstyle :zen:plugins:ssh-agent helper ksshaskpass
```
...
```zsh
zstyle :zen:plugins:ssh-agent identities id_rsa id_rsa2 id_github
```
...
```zsh
zstyle :zen:plugins:ssh-agent identities ~/.config/ssh/id_rsa ~/.config/ssh/id_rsa2 ~/.config/ssh/id_github
# which can be simplified to
zstyle :zen:plugins:ssh-agent identities ~/.config/ssh/{id_rsa,id_rsa2,id_github}
```
...
```zsh
zstyle :zen:plugins:ssh-agent lazy yes
```
...
```zsh
zstyle :zen:plugins:ssh-agent lifetime 4h
```
...
```zsh
zstyle :zen:plugins:ssh-agent ssh-add-args -K -c -a /run/user/1000/ssh-auth
```
...
```
zstyle :zen:plugins:ssh-agent ssh-add-args --apple-load-keychain
```
